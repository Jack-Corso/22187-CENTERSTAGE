...
@TeleOp(name = "TeleOpAllP1")
public class DriverControlAll1 extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive()) {
            ...
            // control intake
           // intake.setPower((int)gamepad1.right_trigger - (int) gamepad1.left_trigger);
            ...
            // control hanging
            if (gamepad1.b) hang.setPower(1);
            else if (gamepad1.a) hang.setPower(-1);
            else hang.setPower(0);

            // control slide
            if (gamepad1.y) slide.setSlide(1);
            else if (gamepad1.x) slide.setSlide(-1);
            else slide.setSlide(0);

//            // control outtake box door
//            if (gamepad1.left_bumper) box.setDoor(PixelBox.doorPositions.open);
//            else if (gamepad1.right_bumper) box.setDoor(PixelBox.doorPositions.close);
//
//            // control outtake box rotation
//            if (gamepad1.dpad_left) box.setRotate(Claw.CLOSE);
//            else if (gamepad1.dpad_right) box.setRotate(Claw.OPEN);
            //claw
            if (gamepad1.dpad_up && gamepad1.left_stick_button && gamepad1.right_stick_button) {
